...
fn main() {
    ...
    // Note: With FIFO, accessing 2 in call 4 doesn't move it to the end
    ...
    // Call 8: hit -> cache: [4,2]
    println!("Calling compute_square(4)...");
    let result = compute_square(4);
    assert_eq!(result, 16);
    println!("Result: {} (should be cached)\n", result);

    // Call 9: miss -> cache: [2,1] (evicts 4 - FIFO)
    println!("Calling compute_square(1)...");
    let result = compute_square(1);
    assert_eq!(result, 1);
    println!("Result: {}\n", result);
    ...
    // Expected: 6 executions (1, 2, 3, 4, 2, 1)
    // Hits: compute_square(2) in call 4, compute_square(3) in call 6, compute_square(4) in call 8
    assert_eq!(
        exec_count, 6,
